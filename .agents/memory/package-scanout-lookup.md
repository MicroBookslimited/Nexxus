...
3. **Duplicate rows shadowing the collectable one.** `/packages/lookup/:tracking` uses suffix-tolerant matching + `limit(1)`; it must ORDER BY status='received' first then newest receivedAt, or a collected/cancelled twin (shared AWB, re-received parcel) returns instead and POS says "unavailable".
