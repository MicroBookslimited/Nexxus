...
*   **Optional Modules: Layaway + Work Orders** — Opt-in modules via tenant settings (`layaway_enabled`, `work_orders_enabled`), gated in nav AND at page level. Layaway reserves stock at create, payoff writes a completed order; work orders charge via a POS handoff that marks them collected. [Details](docs/architecture.md#optional-modules-layaway--work-orders).
*   **Fixed Assets + Technician Teams** — Opt-in asset register (`fixed_assets_enabled`): cost, straight-line depreciation computed on read, condition, photos, scannable AST- tags, service due dates. Assets flagged `isTool` are signed out to a technician or a team through the `asset_assignments` custody ledger, or dispatched on a work order. Teams (`technician_teams`) can be assigned to a work order and expand into `assignedStaffIds`; leaders see their team's jobs in FSM. [Details](docs/architecture.md#fixed-assets--technician-teams).
*   **Package / Shipping Service** — Opt-in module (`packages_enabled`, web only): packages scanned IN at receiving with a per-package fee, scanned OUT at POS like a product; tracking numbers unique per tenant, collectible exactly once (row-locked collect on checkout success). [Details](docs/architecture.md#package--shipping-service).
