...
function colDef(col) {
  ...
  if (col.hasDefault && col.default !== undefined) {
    if (typeof col.default === "string") {
      ...
      emittedDefault = true;
    } else if (col.default && typeof col.default === "object" && "queryChunks" in col.default) {
      // sql`...` template (e.g. defaultNow uses sql`now()`). For timestamp/date
      // columns emit `DEFAULT now()` so NOT NULL inserts that pass `default`
      // do not crash. Without this the live DB column has no default and
      // Drizzle's INSERT … values (..., default) errors with not-null.
      const sqlType = pgType(col).toLowerCase();
      if (/^(timestamp|date|time)/.test(sqlType)) {
        parts.push("DEFAULT now()");
        emittedDefault = true;
      }
    }
  }
  // defaultFn (JS function executed per insert) — for timestamps, prefer DB now().
  if (!emittedDefault && col.defaultFn) {
    const sqlType = pgType(col).toLowerCase();
    if (/^(timestamp|date|time)/.test(sqlType)) {
      parts.push("DEFAULT now()");
    }
